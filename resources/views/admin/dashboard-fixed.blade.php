        ...
        <div class="filter-bar">
            <form id="dashboardFilterForm" action="{{ route('admin.dashboard') }}" method="GET">
                <div class="row align-items-center">
                    ...
                    <!-- Cluster (Hidden) -->
                    <input type="hidden" id="cluster_id" name="cluster_id" value="{{ $clusterId }}">
                    ...
                    <div class="col-md-3 mb-2 mb-md-0 text-end">
                        ...
                        <button type="button" class="btn btn-outline-secondary btn-sm" onclick="window.location.href='{{ route('admin.dashboard') }}'">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeCharts() {
        ...
        submissionTypeChart = new Chart(submissionTypeCtx, {
            type: 'doughnut',
            data: {
    ...
    function updateCharts(data) {
        ...
        // Update submission type chart
        submissionTypeChart.data.datasets[0].data = [
            data.weeklyCount,
            data.monthlyCount,
            data.quarterlyCount,
            data.semestralCount,
        ...
        submissionTypeChart.update();

        // Update monthly trend chart
        monthlyTrendChart.data.datasets[0].data = data.submissionsByMonth;
        monthlyTrendChart.update();

        // Update cluster cards
        const clusterCardsContainer = document.querySelector('.cluster-cards');
        ...
        Object.entries(data.clusterSubmissions).forEach(([clusterName, submissionCount]) => {
            ...
            clusterCard.className = 'col-md-6 mb-3';
            clusterCard.innerHTML = `
                <div class="cluster-card cluster-${clusterId}-card"
                     data-cluster-id="${clusterId}"
                     onclick="filterByCluster(${clusterId})">
                    <div class="cluster-icon">
                        <i class="fas fa-layer-group"></i>
                    </div>
                    <div class="cluster-content">
            ...
            `;
            clusterCardsContainer.appendChild(clusterCard);
        });

        // Reapply active class to the selected cluster card if any
        const activeClusterId = document.getElementById('cluster_id').value;
        if (activeClusterId) {
            const activeCard = document.querySelector(`.cluster-card[data-cluster-id="${activeClusterId}"]`);
            if (activeCard) {
                activeCard.classList.add('active');
            }
        }
